...
{
	// Pre: cert; // Post: és la data d/m/a en cas que sigui correcta i 1/1/2001 en	cas contrari 
...
void Data::mostrar(int format) const
{
	...
	if (format == 1)
		...
		cout << a_dia << '-' << NOM_MES[a_mes] << '-' << a_any;
}
...
void Data::llegir(int format)
{
	...
	int any, mes, dia;
	do
	{					 // cal declarar les variables que es facin servir
...
{
	// Pre: cert;
	// Post: cert si dia/mes/any seria una data vàlida en el calendari gregorià(entre l’any 1600 i 4000), fals altrament 
	bool correcte = true;
	...
	{
		int dies_mes;
		if (es_de_traspas and mes == 2)
			dies_mes = 29;
		else
			dies_mes = DIES_MES[mes];
		if (dia > dies_mes)
			correcte = false;
	}
	return correcte;
